...
class AuthService(val client: ApiClient) {
    ...
    suspend fun register(email: String, password: String, role: String = "USER"): Boolean = withContext(Dispatchers.IO) {
        val req = RegisterRequest(email, password, role)
        val response: HttpResponse = client.httpClient.post("${client.baseUrl}/api/auth/register") {
            setBody(req)
        }
